  ...
  async function fetchProfit() {
    try {
      const response = await API.get(
        `analytics/profit/?start_date=2024-01-01&end_date=2025-01-01`,
      );
          ...
          <div className="dash_top">
            <div className={"left"}>
              <div className="top_products">
                ...
                {products &&
                  products
                    .filter((item) => products.indexOf(item) < 5)
                    .map((product) => (
                      <div className="product" key={product.id}>
                        <h4>{products.indexOf(product) + 1}</h4> {product.title}
                      </div>
                    ))}
              </div>
              <div className={"tasks"}>
              ...
              <div className={"suppliers"}>
                ...
                <div className={"supplier_container"}>
                  {suppliers.map((supplier) => (
                    <div
                      className={"supplier"}
                      key={suppliers.indexOf(supplier)}
                    >
                      <h4>{supplier.name}</h4>
                      <p>{supplier.phone}</p>
                      <p>{supplier.email}</p>
                ...
                {clientOrders
                  ...
                  .map((order) => (
                    <div className={"order"} key={order.id}>
                      ...
                      <p>
                        {order.order_status === 1 ? "Working" : "Completed"}
                      </p>
